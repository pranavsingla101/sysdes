...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../../../../lib-LVEOQB6Q.mjs")).default;
}
...
var nodeSchema = external_exports.object({
  id: external_exports.string().describe("Unique node identifier, e.g. 'service-api'"),
  label: external_exports.string().describe("Display label for the node"),
  shape: external_exports.enum(NODE_SHAPES).describe(
    "Node shape: rectangle (default), diamond (decision), circle (event), pill (service), cylinder (database), hexagon (external)"
  ),
  colorIndex: external_exports.number().int().min(0).max(7).describe(
    "Color palette index 0-7. 0=neutral, 1=blue, 2=purple, 3=orange, 4=red, 5=pink, 6=green, 7=teal"
  ),
  x: external_exports.number().describe("Horizontal position in canvas units"),
  y: external_exports.number().describe("Vertical position in canvas units"),
  width: external_exports.number().optional().describe("Node width in pixels"),
  height: external_exports.number().optional().describe("Node height in pixels")
});
var edgeSchema = external_exports.object({
  id: external_exports.string().describe("Unique edge identifier"),
...
var designSchema = external_exports.object({
  nodes: external_exports.array(nodeSchema).min(1).max(20),
  edges: external_exports.array(edgeSchema).max(30)
});
...
async function broadcast(liveblocks, roomId, event) {
  try {
    await liveblocks.broadcastEvent(roomId, event);
  } catch (err) {
    logger.warn("Failed to broadcast event", { roomId, eventType: event.type, err });
  }
}
...
Use descriptive labels and connect all services with meaningful edges.`
      ...
      const lenientNodeSchema = nodeSchema.extend({
        colorIndex: external_exports.number().transform((v) => Math.max(0, Math.min(7, Math.floor(v)))),
        shape: external_exports.string().transform((v) => NODE_SHAPES.includes(v) ? v : "rectangle")
      });
      const lenientSchema = designSchema.extend({
        nodes: external_exports.array(lenientNodeSchema).min(1).max(20)
      });
      design = lenientSchema.parse(raw);
    ...
    const nodeOps = design.nodes.map((n) => {
    ...
    const edgeOps = design.edges.filter((e) => {
